...
readArgs =
  Aga{ input = def &= help "This flag defines the problems located in the agda-problems-repository for AGA to solve. It can have one of three values. It can point to a single problem - * problem *, the entire directory of problems including subdirectories and their contents - * dir *, or a list of selected problems from the repository placed in a .json file - * list *. This flag does not have a default value."
      ...
      , conF = "config.json" &=help  "this is a config file, it should be in the current directory or * ~/.agda-gpt-conversation * default value for this flag is config.json" &= typFile
      ...
      } &= summary "##################### adga-gpt-experiment #####################\n\nBefore start, you have to prepare directory with templates. Directory is available in git repositry * /data/templates *  Copy this dir in your currnet executing dix1r, or * ~/.agda-gpt-conversation * witch one you need to create first. \n\nExample: aga -i=dir -s= Problems/ExampleProblems/ -c=myConfig.json -m=Pretty -l=15" &= details ["More details on the website https://formalFoundry.ai"]
